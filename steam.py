...
class steam():
    ...
    def calc(self):
        ...
        sat_properties = np.loadtxt('sat_water_table.txt', skiprows=1)
        ts, ps, hfs, hgs, sfs, sgs, vfs, vgs = sat_properties.T

        superheat_properties = np.loadtxt('superheated_water_table.txt', skiprows=1)
        tcol, hcol, scol, pcol = superheat_properties.T
        ...
        if self.T is not None:
            if self.T > Tsat:  # interpolate with griddata
                ...
                self.h = griddata((tcol, pcol), hcol, (self.T, Pbar))
                self.s = griddata((tcol, pcol), scol, (self.T, Pbar))
                self.x = 1.0
            ...
            if self.x <= 1.0:  # manual interpolation
                ...
                self.region = 'Superheated'
                self.T = griddata((hcol, pcol), tcol, (self.h, Pbar))
                self.s = griddata((hcol, pcol), scol, (self.h, Pbar))
            ...
            if self.x <= 1.0:  # manual interpolation
                ...
                self.region = 'Superheated'
                self.T = griddata((scol, pcol), tcol, (self.s, Pbar))
                self.h = griddata((scol, pcol), hcol, (self.s, Pbar))
            #endregion
