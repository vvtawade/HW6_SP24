...
class Pipe():
    ...
    def Re(self):
        ...
        :return:
        '''
        self.reynolds = (self.rho * self.V() * self.d) / self.mu  # Re=rho*V*d/mu, be sure to use V() so velocity is updated.
    ...
    def frictionHeadLoss(self):  # calculate headloss through a section of pipe in m of fluid
        ...
        hl = (ff * self.length * self.V()**2) / (2 * g) # calculate the head loss in m of water
        return hl
...
class PipeNetwork():
    ...
    def findFlowRates(self):
        ...
        def fn(q):
            ...
            L = np.concatenate((q_net, loop_head_losses))
